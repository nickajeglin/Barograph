...
float maxP = 102500;    //in Pa
float minP = 99000;     //in Pa
byte units = 2;         //units on startup: 0=conventional, 1=metric, 2=mixed (temp:F, pres:hPa)
...
void setup() {
  ...
  printReadings(units, tempC, pressureStack[143]);
}
...
void loop() {
  ...
  if ( (millis() - lastDebounceTime) > debounceDelay) {
    if (digitalRead(unitPin) == LOW) {
      ...
      tempC = baro.getTemperature();
      printReadings(units, tempC, pascals);
      lastDebounceTime = millis();
    }
  }
...
void printReadings(int units, float tempC, float pascals) {
  clearScreen();
  tft.setCursor(0, 0);
  tft.setTextColor(TXTCOLOR);  tft.setTextSize(3);
  switch (units) {
    case 0:
      tft.print((tempC * 1.8) + 32, 1); tft.println(F(" F"));
      tft.print(pascals / 3386.4, 2); tft.println(F(" inHg"));
      break;
    case 1:
      tft.print(tempC, 1); tft.println(F(" C"));
...
void printDeltaP() {
  tft.setTextColor(TXTCOLOR);  tft.setTextSize(3); tft.setCursor(0, 48);
